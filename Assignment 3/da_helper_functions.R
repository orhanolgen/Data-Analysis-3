...
price_diff_by_variables3 <- function(df, factor_var, dummy_var, factor_lab, dummy_lab) {
  # Convert variable names into symbols
  factor_var <- as.name(factor_var)
  dummy_var <- as.name(dummy_var)
  
  # Compute mean price and standard error for each group
  stats <- df %>%
    group_by(!!factor_var, !!dummy_var) %>%
    dplyr::summarize(Mean = mean(price, na.rm = TRUE),
                     se = sd(price, na.rm = TRUE) / sqrt(n()), .groups = 'drop')
  
  # Ensure dummy variable is treated as a factor with levels 0 and 1
  stats[[as.character(dummy_var)]] <- factor(stats[[as.character(dummy_var)]], levels = c(0, 1), labels = c("No", "Yes"))
  
  # Ensure factor variable is treated as a factor with its unique levels
  stats[[as.character(factor_var)]] <- factor(stats[[as.character(factor_var)]])
  
  # Plot mean price differences
  ggplot(stats, aes_string(x = colnames(stats)[1], y = colnames(stats)[3], fill = colnames(stats)[2])) +
    geom_bar(stat = 'identity', position = position_dodge(width = 0.9), alpha = 0.8) +
    geom_errorbar(aes(ymin = Mean - (1.96 * se), ymax = Mean + (1.96 * se)),
                  position = position_dodge(width = 0.9), width = 0.25) +
    scale_color_manual(name = dummy_lab, values = c(color[4], color[5])) +  # Two colors for 0/1 dummy
    scale_fill_manual(name = dummy_lab, values = c(color[4], color[5])) +   # Two colors for 0/1 dummy
    ylab("Mean Price") +
    xlab(factor_lab) +
    theme_bg() +
    theme(panel.grid.major = element_blank(),
